...
def add_dash(server):
    ...
    def produce_individual(api_well_num):
        ...
        index = list(
            range(
                min(points[api_well_num].keys()), max(points[api_well_num].keys()) + 1
            )
        )
        gas = []
        oil = []
    ...
    # Slider -> count graph
    @app.callback(
        Output("year_slider", "value"), [Input("count_graph", "selectedData")]
    )
    ...
    def make_main_figure(
        ...
        if (
            main_graph_layout is not None
            and selector is not None
            and "locked" in selector
        ):
    ...
    @app.callback(
        Output("individual_graph", "figure"), [Input("main_graph", "hoverData")]
    )
