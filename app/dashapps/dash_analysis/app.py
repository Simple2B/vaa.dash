...
def add_dash(server):
    ...
    score_inputs = [
        Input((i + "_score_graph"), "selectedData") for i in all_departments
    ]
    ...
    def description_card():
        ...
        return html.Div(
            ...
            children=[
                ...
                html.Div(
                    id="intro",
                    children="Explore clinic patient volume by time of day, waiting time, and care score. Click on the heatmap to visualize patient experience at different time points.",  # noqa F731
                ),
            ],
        )
    ...
    def generate_patient_volume_heatmap(
        ...
        filtered_df = filtered_df.sort_values("Check-In Time").set_index(
            "Check-In Time"
        )[start:end]

        x_axis = [
            datetime.time(i).strftime("%I %p") for i in range(24)
        ]  # 24hr time list
        y_axis = day_list
    ...
    def create_table_figure(
        ...
        f = lambda x_val: dt.strftime(x_val, "%Y-%m-%d")  # noqa F731
        check_in = (
    ...
    def update_table(start, end, clinic, admit_type, heatmap_click, reset_click, *args):
        ...
        if prop_type != "selectedData" or (
            ...
            selected_patient = ctx.triggered[0]["value"]["points"][0][  # noqa F841
                "customdata"
            ]
